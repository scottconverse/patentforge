...
test.describe('Feasibility Pipeline', () => {
  ...
  test('runs full 6-stage pipeline and shows report', async ({ page, consoleErrors }) => {
    ...
    // Wait for the report to appear (pipeline_complete triggers loadProject → report view)
    await expect(page.locator('text=Feasibility Report')).toBeVisible({ timeout: 30_000 });

    // Verify report content is rendered
    await expect(page.locator('.markdown-content')).toBeVisible();
    const reportText = await page.locator('.markdown-content').textContent();
    expect(reportText).toContain('Technical Intake & Restatement');
    expect(reportText).toContain('Prior Art Research');
    expect(reportText).toContain('Comprehensive Report');

    // Verify disclaimer is present
    await expect(page.locator('text=This report was generated by PatentForge')).toBeVisible();
