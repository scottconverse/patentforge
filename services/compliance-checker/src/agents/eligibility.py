"""
Eligibility Agent — 35 USC 101 check.

Evaluates whether each independent claim satisfies subject matter
eligibility under the Alice/Mayo two-step framework.
"""
...
def _load_prompt() -> str:
    ...
    prompt_path = PROMPTS_DIR / "eligibility.md"
    if prompt_path.exists():
        return common + prompt_path.read_text(encoding="utf-8")
    return common + "You are a patent compliance checker for 35 USC 101 eligibility."
...
async def run_eligibility(state: GraphState) -> GraphState:
    """Check claims against 35 USC 101 eligibility requirements."""
    prompt = _load_prompt()
...
Check each independent claim against 35 USC 101 eligibility requirements (Alice/Mayo framework)."""
    ...
    try:
        ...
        state.eligibility_results = json.dumps(results)
    except (json.JSONDecodeError, ValueError) as e:
        state.error = f"Eligibility check returned invalid JSON: {e}"

    state.step = "eligibility_complete"
    return state
